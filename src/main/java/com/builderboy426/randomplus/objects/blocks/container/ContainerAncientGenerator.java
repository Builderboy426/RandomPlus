...
public class ContainerAncientGenerator extends Container {
	
	private final TileEntityAncientGenerator tileEntity;
	private int energy, cookTime;
	
	public ContainerAncientGenerator(InventoryPlayer player, TileEntityAncientGenerator tileEntity) {
		this.tileEntity = tileEntity;
		IItemHandler handler = tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);
		
		this.addSlotToContainer(new SlotItemHandler(handler, 0, 87, 21));
		//this.addSlotToContainer(new SlotItemHandler(handler, 1, 87, 42));
